...
def cylindricalWarpImage(img1, K, savefig=False):
    ...
    for x_cyl in np.arange(0,cyl_w):
        for y_cyl in np.arange(0,cyl_h):
            ...
            X = np.array([np.sin(theta), h, np.cos(theta)])
            X = np.dot(K,X)
            x_im = X[0] / X[2]
...
def transformImage(src, dst, method = 'affine'):

    (M, pts1, pts2, mask)= getTransform(src, dst, method)
    rows,cols = dst.shape
    if method == 'homography':
        return cv2.warpPerspective(src, M, (cols, rows))
    if method == 'affine':
        return cv2.warpAffine(src, M[:2,:], (cols, rows))
...
def stitchImage(img1, img2):
    non_cross_mask = cv2.bitwise_xor(np.array(img1>0, dtype=np.uint8),np.array(img2>0, dtype=np.uint8))
    cross_mask = cv2.bitwise_and(np.array(img1>0, dtype=np.uint8),np.array(img2>0, dtype=np.uint8))
    non_cross_part = cv2.add(img1,img2, mask = non_cross_mask)
    cross_part = cv2.add(np.zeros(img1.shape, dtype = np.uint8), img1, mask = cross_mask)
    output_image = cv2.add(non_cross_part, cross_part)
    return output_image


# ============ 2. Stitch using Laplacian Pyramid =========
'''
this function is from the blog of Roy
'''
def Laplacian_Pyramid_Blending_with_mask(A, B, m, num_levels = 6):
    ...
    for i in xrange(num_levels-1,0,-1):
        ...
        sizeA = (gpA[i-1].shape[1], gpA[i-1].shape[0])
        sizeB = (gpB[i-1].shape[1], gpB[i-1].shape[0])
        LA = np.subtract(gpA[i-1], cv2.pyrUp(gpA[i], dstsize = sizeA))
        LB = np.subtract(gpB[i-1], cv2.pyrUp(gpB[i], dstsize = sizeB))
        lpA.append(LA)
        lpB.append(LB)
...
# find the centerline of overlap
def findCenterCol(mask):
    center_row = mask.shape[0] / 2
    start = 0
    end = 0
    for i in range(0, mask.shape[1]-1):
        if mask[center_row, i] == 0 and mask[center_row, i + 1] == 1:
            start = i
        elif mask[center_row, i] == 1 and mask[center_row, i + 1] == 0:
            end = i
    return (start + end )/2

# stitch using lgb
def stitchImageLgb(img1, img2):
    cross_mask = cv2.bitwise_and(np.array(img1>0, dtype=np.uint8),np.array(img2>0, dtype=np.uint8))
    mask = np.zeros_like(img1, dtype=np.float32)
    mask[:, findCenterCol(cross_mask):] = 1.
    output_image = Laplacian_Pyramid_Blending_with_mask(img1, img2, m=mask)
    return output_image
...
def Perspective_warping(img1, img2, img3):
    ...
    # extend the domain of the center img.
    output_image = cv2.copyMakeBorder(img1,200,200,500,500, cv2.BORDER_CONSTANT)

    # transfer the outer two images
    patch1 = transformImage(img2, output_image, 'homography')
    patch2 = transformImage(img3, output_image, 'homography')

    
    # blend images
    output_image = stitchImage(patch1, output_image)
    output_image = stitchImage(patch2, output_image)
...
def Bonus_perspective_warping(img1, img2, img3):
    ...
    # blend images
    output_image = stitchImageLgb(patch1, output_image)
    output_image = stitchImageLgb(output_image, patch2)
...
def Cylindrical_warping(img1, img2, img3):
    ...
    for img in [img1, img2, img3]:
        h,w = img.shape
        K = np.array([[f, 0, w/2], [0, f, h/2], [0, 0, 1]]) # mock calibration matrix
        img = cylindricalWarpImage(img, K)[0]
        # extend the domain of the center img.
        img = cv2.copyMakeBorder(img,50,50,300,300, cv2.BORDER_CONSTANT)
        images.append(img)
    
    
    # Transform the pictures
    output_image = images[0]
    patch1 = transformImage(images[1], output_image, 'affine')
    patch2 = transformImage(images[2], output_image, 'affine')
